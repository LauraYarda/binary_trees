...
/**
 * enqueue_item_4 - It adds an item to a queue.
 * @queue_h: The pointer to the queue's head.
 * @queue_t: The pointer to the queue's tail.
 * @n: The pointer to the queue's size value.
...
/**
 * get_last_heap_node - It gets the last level order node \
 * in a max binary heap tree.
 * @root: The root of the max binary heap tree.
 *
 * Return: The pointer to the last level order node, otherwise NULL.
 */
 ...
 * @node: The pointer to the node's address.
 ...
 * @root: The pointer to the address of the tree's root node.
 *
